...
class Trainer:
    ...
    def train(self) -> Dict[str, object]:
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            number = ' '
            np.save('results/losses_train_{}'.format(number), self.losses_train)
            np.save('results/f1_train_{}'.format(number), self.f1s_train)
            if self._validation_data is not None:
                ...
                np.save('results/losses_{}'.format(number), self.losses_valid)
                np.save('results/f1_{}'.format(number), self.f1s_valid)
    ...
    def _save_checkpoint(self,
        ...
        if self._serialization_dir is not None:
            ser_dir_now = self._serialization_dir + '_' + str(epoch)
